...
#--------------CHARGEMENT DES CONFIGS--------------------------------------------
    ...
    def onEvent(self, event):
        ...
        if event.type == b3.events.EVT_CLIENT_SAY or event.type == b3.events.EVT_CLIENT_TEAM_SAY or event.type == b3.events.EVT_CLIENT_PRIVATE_SAY:
            self.AFKSpeak(event)
    ...
    def cmd_ChangePlayer(self, data, client, cmd=None):
        ...
        if input[1] != None:   
            ...
            if client1 and client2 and client.groupBits > client1.groupBits and client.groupBits > client2.groupBits :
                if client2.team==b3.TEAM_RED:
                    self.console.write('forceteam %s %s' % (client2.cid, 'BLUE'))
                elif client2.team==b3.TEAM_BLUE:
                ...
                elif client1.team==b3.TEAM_BLUE:
...
#--------------COMMANDE AFK-KAFK-------------------------------------------
    ...
    def cmd_AFK(self, data, client, cmd=None):
        ...
        if not data:
            ...
            return
        # input[0] is the player id
        clientafk = self._adminPlugin.findClientPrompt(input[0], client)
        if clientafk:
...
#--------------COMMANDE PM--------------------------------------------
    ...
    def cmd_PM(self, data, client, cmd=None):
        ...
        if not data:
            ...
            return
        # input[0] is the player id
        clientpm = self._adminPlugin.findClientPrompt(input[0], client)
        # Envoi du message
        if len(input[1]):
            Texte = input[1]
...
#--------------COMMANDE SALUT--------------------------------------------
    ...
    def cmd_Salut(self, data, client, cmd=None):
        ...
        if not data:
            ...
            return
        # input[0] is the player id
        clientHello = self._adminPlugin.findClientPrompt(input[0], client)
        # Envoi du bigtext
        if clientHello:
...
#--------------AUTONUKE--------------------------------------------
    ...
    def cmd_autonuke(self, data, client, cmd=None):
        ...
        if not data:
            ...
            return
        # input[0] is the player id
        self.clientautonuke = self._adminPlugin.findClientPrompt(input[0], client)
