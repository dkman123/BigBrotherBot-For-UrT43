        ...
        if client.maxLevel < self.ignorelevel:
            if hitloc_kills >= self.treshold:  # Set it to greater or equal to just in case rcon send fails
                ...
                if self.action == 0:
                    ...
                    self.addFollow(client)
                    client.tempban(reason=self.kickmessage, keyword="aimbotdetector", duration=self.duration,
                                   data="%s kills" % hitloc_kills)
                elif self.action == 2:
...
if __name__ == '__main__':
    ...
    #need an ending backslash on the next line
    conf.setXml('''
